...
import { resolveSymbol } from '../pair-mapping.js';

/**
 * Aggregate multiple raw samples into one MinuteRow per exchange+ticker.
 *
 * Rules (from plan section 6.2):
 * - Numeric fields: median across successful samples only
 * - Boolean fields: majority vote (true count > false count), tie → false
 * - Meta fields: see individual rules below
 * - Zero successful samples: write row with null metrics + error
 */
export function aggregateMinute(
  ...
  for (const [key, groupSamples] of groups) {
    ...
    if (analyses.length === 0) {
      // Full failure — write row with null metrics
...
function buildRow(
  ...
  return {
    ...
    // Slippage — indexed by NOTIONAL_TIERS order
    ...
    // Filled notional for the $1M tier
    ...
    // Meta: aggregated estimate — true if any sample is aggregated
    is_aggregated_estimate: analyses.some((a) => a.isAggregatedEstimate === true),

    // Hyperliquid meta — minimum across successful samples
    ...
    // Lighter meta — true if any sample used WS
...
function makeNullRow(
  ...
  return {
    ...
    bid_fill_100k: null,
    bid_fill_1m: null,
    ask_filled_notional_1m: null,
    bid_filled_notional_1m: null,
...
function majorityVote(values: boolean[]): boolean {
  ...
  return trueCount > values.length / 2; // tie → false
}

/**
 * Per-tier minimum nSigFigs across all analyses.
 */
